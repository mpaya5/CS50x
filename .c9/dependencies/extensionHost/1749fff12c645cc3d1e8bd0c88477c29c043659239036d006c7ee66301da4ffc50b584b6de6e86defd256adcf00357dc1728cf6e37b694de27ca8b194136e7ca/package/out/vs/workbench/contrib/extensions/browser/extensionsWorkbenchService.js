...
`);
    ...
    let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends lifecycle_1.Disposable {
        constructor(instantiationService, editorService, extensionService, galleryService, configurationService, telemetryService, notificationService, urlService, extensionEnablementService, windowService, progressService, extensionManagementServerService, storageService, modeService, productService) {
            super();
            this.instantiationService = instantiationService;
        ...
        checkAndEnableDisabledDependencies(extensionIdentifier) {
            ...
            if (extension) {
                const disabledDepencies = this.getExtensionsRecursively([extension], this.local, 4 /* EnabledGlobally */, { dependencies: true, pack: false });
                if (disabledDepencies.length) {
                    return this.setEnablement(disabledDepencies, 4 /* EnabledGlobally */);
                }
            }
            return Promise.resolve();
        }
    ...
    ExtensionsWorkbenchService = __decorate([
        ...
        __param(14, product_1.IProductService)
